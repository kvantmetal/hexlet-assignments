...
public class GuestsController {
    ...
    // BEGIN
    @PostMapping(path = "")
    public ResponseEntity<GuestDTO> saveGuest(@Valid @RequestBody GuestCreateDTO guestCreateDTO) {
        Guest guest = guestMapper.map(guestCreateDTO);
        guestRepository.save(guest);
        return ResponseEntity.status(HttpStatus.CREATED).body(guestMapper.map(guest));
    }
    // END
}
